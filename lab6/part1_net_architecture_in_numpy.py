...
def zad2_two_layer_net(student_id):
    ...
    class SimpleTwoLayerNetwork:
        ...
        def forward(self, x_data):
            layer_hidden_output = self.hidden_layer.forward(x_data)
            return self.output_layer.forward(layer_hidden_output)
    ...
    model.hidden_layer.W[:, 0] = [2.0, 2.4]      # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [-0.5, -0.7]      # wagi neuronu h2
    model.hidden_layer.b[:] = [-1.0, 1.0]         # biasy neuronów h1 i h2
    model.output_layer.W[:, 0] = [0.7, 0.7]      # wagi neuronu wyjściowego
...
if __name__ == '__main__':
    ...
    student_id = 193195         # Twój numer indeksu, np. 102247
